...
/*
Complexity Analysis:
1. minElement - Time: O(n), Space: O(n)
2. sumElements - Time: O(n), Space: O(n)
3. isPrime - Time: O(√n), Space: O(√n)
4. factorial - Time: O(n), Space: O(n)
5. fibonacci - Time: O(2^n), Space: O(n)
6. power - Time: O(n), Space: O(n)
7. generatePermutations - Time: O(n!), Space: O(n!)
8. containsOnlyDigits - Time: O(n), Space: O(n)
9. binomialCoefficient - Time: O(2^n), Space: O(n)
10. gcd - Time: O(log n), Space: O(log n)
*/
